...
public class InMemoryWorldDao implements WorldDao {
    ...
    public static void main(String args[]) {
        ...
        //List<String> cities= allCities.entrySet().stream().map(e->e.getValue().getCountryCode()).collect(Collectors.toList());
       // System.out.println(cities);
       // List<String> highestPopulated= allCountries.entrySet().stream().map(e->e.getValue().getCode()).collect(Collectors.toList());

        ////////////////Find the highest populated city of each country
        Map<String, City> highestPopulated = new HashMap<>();

        allCountries.entrySet()
                .stream()
                .filter(entry -> entry != null && entry.getValue() != null && entry.getValue().getCities() != null)
                .forEach(entry -> {
                    String countryName = entry.getValue().getName();
                    City highestCity = entry.getValue().getCities()
                            .stream()
                            .filter(city -> city != null)
                            .max(Comparator.comparingInt(City::getPopulation))
                            .orElse(null);

                    if (countryName != null && highestCity != null) {
                        highestPopulated.put(countryName, highestCity);
                    }
                });
        for(Map.Entry<String,City> e :highestPopulated.entrySet()){
            System.out.println("Country: "+e.getKey() + " City : " + e.getValue());
        }
        ////////////////////Find the most populated city of each continent
        Map<String, City> mostPopulatedCitiesByContinent = allCountries.values().stream()
                .filter(country -> country != null && country.getCities() != null)
                .flatMap(country -> country.getCities().stream().map(city -> Map.entry(country.getContinent(), city)))
        ...
        for (String continent : allContinents) {
            City mostPopulatedCity = mostPopulatedCitiesByContinent.get(continent);
            System.out.println("Continent: " + continent + ", Most Populated City: " + mostPopulatedCity);
        }
        //////////// Find the highest populated capital city
        Map<String, City> highestPopulatedCapital = new HashMap<>();

       City highestPopulatedCapitalCity = allCountries.entrySet()
                .stream()
                .filter(entry -> entry != null && entry.getValue() != null && entry.getValue().getCities() != null)
               .map(entry -> entry.getValue().getCities()
                       .stream()
